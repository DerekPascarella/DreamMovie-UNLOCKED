...
#
# Patches FLAG.PVR into a previously-patched DreamMovie CDI (in-place).
# Expects FLAG.PVR and a "DreamMovie (Patched *.cdi" in this folder.
#
# FLAG.PVR is a 512x512 ARGB1555 twiddled PVR texture, 524320 bytes.
# In the CDI it's found by matching GBIX+PVRT headers with pixel format
# 0x02 and data format 0x01. DISK.PVR uses the same format but comes
...
# PVR header bytes that identify FLAG.PVR
PIXEL_FORMAT = 0x02  # ARGB1555
DATA_FORMAT = 0x01   # twiddled
SIBLING_NAME = "DISK.PVR"  # same format, comes first on disc
...
def main():
    ...
    pvr_path = os.path.join(script_dir, "FLAG.PVR")
    if not os.path.isfile(pvr_path):
        print(f"Error: FLAG.PVR not found in {script_dir}")
        sys.exit(1)

    pvr_size = os.path.getsize(pvr_path)
    if pvr_size != EXPECTED_SIZE:
        print(f"Error: FLAG.PVR is {pvr_size} bytes, expected {EXPECTED_SIZE}")
        sys.exit(1)

    cdi_path = find_patched_cdi(script_dir)
    print(f"FLAG.PVR: {pvr_size:,} bytes")
    ...
    # Locate FLAG.PVR in the CDI
    print("Searching for FLAG.PVR in CDI...")
    ...
    if len(candidates) == 0:
        ...
        print(f"  Warning: only 1 match (expected 2: {SIBLING_NAME} + FLAG.PVR)")
        sector_start = candidates[0]
    else:
        print(f"  Found {len(candidates)} matches, selecting #2 as FLAG.PVR")
        sector_start = candidates[1]

    data_start = sector_start + SUBHEADER_SIZE
    print(f"  FLAG.PVR at CDI offset {data_start} (sector start: {sector_start})")
